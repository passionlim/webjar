...
/**
    ...
    jindo.TYPE;    // 버전 타입 문자열 (desktop|m) - ex. "desktop"
*/
...
/**
	...
	@remark 
		<ul class="disc">
			<li>1.4.3 버전부터 m,msafari,mopera,mie 사용 가능.</li>
...
/**
	...
	@param {Element+} elTemplate 템플릿으로 HTML 요소. 파라미터가 HTML 요소이면 TEXTAREA와 SCRIPT 요소만 사용할 수 있다.
		<ul class="disc">
			<li>HTML 요소가 SCRIPT 요소인 경우 type 속성은 반드시 "text/tmpl"으로 지정해야 한다.</li>
	...
	@example
		<script type="text/tmpl" id="tpl3">
			<%=service%> : <%=url%>
		</script>
		
		// Micro-Templating 엔진을 사용하는 인스턴스로 생성.
		var tmpl = $Template("tpl3", "micro");
	@example
		<script type="text/tmpl" id="tpl4">
			{{service}} : {{url}}
		</script>
		
		// handlebars 엔진을 사용하는 인스턴스로 생성.
		var tmpl = $Template("tpl4", "handlebars");
	@example
		<script type="text/tmpl" id="tpl5">
			{{service}} : {{url}}
		</script>
		
		// simple 엔진을 사용하는 인스턴스로 생성.
		var tmpl = $Template("tpl5", "simple");
	@example
		<script type="text/tmpl" id="tpl6">
			{=service} : {=url}
		</script>
...
jindo.$Template = function(str, sEngineName){
	...
	if((obj = document.getElementById(str) || str) && obj.tagName && (tag = obj.tagName.toUpperCase()) && (tag == "TEXTAREA" || (tag == "SCRIPT" && obj.getAttribute("type") == "text/tmpl"))){
